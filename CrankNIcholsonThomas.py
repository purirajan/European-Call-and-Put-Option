...
def CrankNicholsonThomas(X_max,X_min,dx,T,dt,K,r,vol,delta,S):
    N = int((X_max-X_min)/dx)
    M = int(T/dt)
    X = np.zeros(N-1) # Stock Price Discretization
    ...
    #print(Matrix)
    b1= np.zeros((N-1,N-1)) # matrix of size N-1*N-1 as like Ax=b
    b2= np.zeros((N-1,N-1))
    for i in xrange(0,M):
