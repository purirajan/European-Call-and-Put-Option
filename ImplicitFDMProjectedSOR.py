...
def ImplicitFDMProjectedSOR(X_max,X_min,dx,T,dt,K,r,vol,delta,S,w,Kmax):
    ...
    #print(Matrix)
    for i in range(M):
        V_call = SOR(Matrix,V_call,V_call +F,N-1,w, Kmax)
        V_put = SOR(Matrix,V_call,V_put +F,N-1,w, Kmax)
        for j in range(N-1):
